		...
		<>
			...
			>
				<Form className="flex flex-1 flex-col items-end" noValidate={true}>
					<div className="grid w-full grid-cols-[125px,_1fr] gap-[20px] sm:grid-cols-2">
						<div className="relative mb-[20px] w-full">
							...
							<button
								{...getToggleButtonProps({
									type: "button",
									className: `flex w-full cursor-pointer items-center justify-between rounded-[5px] border-[1px] border-secondary p-[5px] pl-[10px] text-secondary ${isOpen ? "rounded-b-none" : null
										}`,
								})}
							>
								{selectedItem}
							...
							<ul
								{...getMenuProps({
									className: `${isOpen ? "scale-y-1" : "scale-y-0"
								...
								{months.map((item, index) => (
									<li
										{...getItemProps({
											item,
											key: item,
											className: `cursor-pointer border-b-[1px] border-secondary px-[10px] py-[5px] last:border-b-0 ${item == selectedItem || highlightedIndex == index
													? "bg-dimmedWhite"
													: null
												}`,
										})}
					...
					<button
						type="submit"
						className={`mt-auto h-[40px] w-[195px] cursor-pointer rounded-[5px] font-normal text-white ${isLocked ? "bg-lightGray" : "bg-primary"
